...
export class DefaultComponent implements OnInit, AfterViewInit {
  ...
  public getImportantNotifyResult: any;
  public getImportantMsg: any;
  private notifyStatus: number;
  private notifyData: any;
  public notifyLists: any;
  userInfo: any;
  userId: any;
  userChildIds: string;
  companyId: any;
  componentId: any;
    ...
    private _getImportantPoliciesURL = '/notification/notification.php?notifyApi=1&action=1&notifyAction=lists&limit=5';
    constructor(private servicePNotify: NotificationsService, private httpClient: HttpClient, private app: AppComponent, private globalapp: GlobalApp) {
     }

    ngOnInit() {
            setTimeout(() => {
            const userInfo  = this.globalapp.getInfo();
            const companyId = userInfo.company_id;
            const userId    = userInfo.user_id;
            const childIds  = userInfo.user_child_id;
            const componentId   = this.globalapp.COMPONENTID;
            this.getImportantNotification(companyId, componentId, userId, childIds)
                .subscribe((data) => {
                    this.notifyData = data;
                    this.notifyStatus = this.notifyData.status;
                    if (this.notifyStatus === 200) {
                        this.getImportantNotifyResult = this.notifyData.rows;
                    } else {
                        this.getImportantMsg = 'No Notification Found';
                    }
                });
     ...
     getImportantNotification(companyId: string, componentId: string, userId: string, childIds: string) {
        let Params = new HttpParams();
        //this.app.BASE_URL
        Params = Params.append('cid', companyId);
        Params = Params.append('compnt_id', componentId);
        Params = Params.append('user_id', userId);
        Params = Params.append('chid', childIds);
        return this.httpClient.get(this.app.BASE_URL + this._getImportantPoliciesURL,{ params : Params });
    }
